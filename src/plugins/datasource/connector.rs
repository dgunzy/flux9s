    ...
    match config.source_type {
        DataSourceType::KubernetesService => {
            let client = kube_client
                .ok_or_else(|| anyhow::anyhow!("Kubernetes client required for kubernetes_service data source"))?;

            Ok(Box::new(super::k8s_service::KubernetesServiceDataSource::new(
                client,
                config.service.clone().ok_or_else(|| anyhow::anyhow!("service field required"))?,
                config.namespace.clone().ok_or_else(|| anyhow::anyhow!("namespace field required"))?,
                config.port.ok_or_else(|| anyhow::anyhow!("port field required"))?,
                config.path.clone().ok_or_else(|| anyhow::anyhow!("path field required"))?,
            )?))
        }
        DataSourceType::Http => {
            Ok(Box::new(super::http::HttpDataSource::new(config)?))
        }
        DataSourceType::File => {
            Ok(Box::new(super::file::FileDataSource::new(
                config.file_path.clone().ok_or_else(|| anyhow::anyhow!("file_path field required"))?,
            )?))
        }
